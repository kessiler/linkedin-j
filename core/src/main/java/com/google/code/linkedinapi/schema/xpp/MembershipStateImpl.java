...
extends BaseSchemaEntity    implements MembershipState
{
    ...
    public void init(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == XmlPullParser.START_TAG) {
            ...
            if (name.equals("code")) {
                setCode(MembershipStateCode.fromValue(XppUtils.getElementValueFromNode(parser)));
            } else {
    ...
    public void toXml(XmlSerializer serializer) throws IOException {
        XmlSerializer element = serializer.startTag(null, "membership-state");
        if (getCode() != null) {
            XppUtils.setElementValueToNode(element, "code", getCode().value());
        }
        
        
        serializer.endTag(null, "membership-state");
    }
}
