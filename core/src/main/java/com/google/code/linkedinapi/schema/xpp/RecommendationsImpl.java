...
extends BaseSchemaEntity implements Recommendations
{
    ...
    public void init(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == XmlPullParser.START_TAG) {
            ...
            if (name.equals("recommendation")) {
                RecommendationImpl node = new RecommendationImpl();
                node.init(parser);
                getRecommendationList().add(node);
            } else {
    ...
    public void toXml(XmlSerializer serializer) throws IOException {
        XmlSerializer element = serializer.startTag(null, "recommendations");
        XppUtils.setAttributeValueToNode(element, "count", getCount());
        XppUtils.setAttributeValueToNode(element, "start", getStart());
        for (Recommendation node : getRecommendationList()) {
            ((RecommendationImpl) node).toXml(serializer);
        }
        
        
        serializer.endTag(null, "recommendations");
    }
}
