...
extends BaseSchemaEntity    implements GroupMemberships
{
    ...
    public void init(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == XmlPullParser.START_TAG) {
            ...
            if (name.equals("group-membership")) {
                GroupMembershipImpl node = new GroupMembershipImpl();
                node.init(parser);
                getGroupMembershipList().add(node);
            } else {
    ...
    public void toXml(XmlSerializer serializer) throws IOException {
        XmlSerializer element = serializer.startTag(null, "group-memberships");
        ...
        for (GroupMembership node : getGroupMembershipList()) {
            ((GroupMembershipImpl) node).toXml(serializer);
        }
        
        
        serializer.endTag(null, "group-memberships");
    }
}
