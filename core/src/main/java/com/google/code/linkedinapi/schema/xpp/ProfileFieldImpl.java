    ...
    implements ProfileField
{
	...
	public void init(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == XmlPullParser.START_TAG) {
        	...
        	if (name.equals("code")) {
        		setCode(ProfileFieldCode.fromValue(XppUtils.getElementValueFromNode(parser)));
            } else {
	...
	public void toXml(XmlSerializer serializer) throws IOException {
		XmlSerializer element = serializer.startTag(null, "profile-field");
        if (getCode() != null) {
            XppUtils.setElementValueToNode(element, "code", getCode().value());
        }
		element.endTag(null, "profile-field");;
	}
}
